...
import Debug.Trace
import Foreign
import Foreign (Ptr, StablePtr, nullPtr)
...
fmi2Instantiate _ _ _ _ ptrCbFuncs _ _ = do
  ...
  let loggingFunction = FMIT.createLoggingFunction cbFuncs "haskellFMU"
   in do
        loggingFunction FMIT.Ok "HS-Message: Example Instantiate - logOk"
        -- Retrieve the user defined state
        state <- getSetupImpure setupVar
        case state of
          -- The user has not provided a state
          Nothing ->
            loggingFunction FMIT.Error "HS-Message: Instantiate - no state set"
              >> (return . castPtrToStablePtr) nullPtr
          -- The user has provided a state
          Just s -> do
            ioref <-
              newIORef
                FMIT.FMIComponent
                  { fcVars = T.sSVs s,
                    fcDoStep = T.sDoStepFunc s,
                    fcEndTime = Nothing,
                    fcState = FMIT.Instantiated,
                    fcPeriod = T.sPeriod s,
                    fcRemTime = T.sPeriod s,
                    fcUserState = T.sUserState s,
                    fcLoggingFunction = loggingFunction
                  }
            newStablePtr ioref
...
setLogicImpure comp varRefs size varVals varValConvF =
  do
    ...
    let varVals'' :: [T.SVTypeVal] = map varValConvF varVals'
        varRefs'' = map fromIntegral varRefs'
        stateStatus = setLogic state varRefs'' varVals''
     in updStateCalcStatusImpure comp stateStatus
...
setLogic state@FMIT.FMIComponent {fcVars = vs} vRefs vVals =
  let refVals = zip vRefs vVals
      ys' = foldr updateVal vs refVals
   in (state {FMIT.fcVars = ys'}, T.OK)
  where
...
fmi2GetBoolean comp varRefs size varVals =
  let valToCBool (x :: T.SVTypeVal) =
        case x of
          ...
          _ -> Nothing
   in getLogicImpure comp varRefs size varVals valToCBool
...
fmi2GetReal comp varRefs size varVals =
  let valToCDouble (x :: T.SVTypeVal) =
        case x of
          ...
          _ -> Nothing
   in getLogicImpure comp varRefs size varVals valToCDouble

-- | The function 'getLogicImpure' extract the relevant values from the user state and wraps them in correct C types
...
getLogicImpure comp varRefs size varVals toVarValF = do
  state <- getStateImpure comp
  varRefs' :: [CUInt] <- peekArray (fromIntegral size) varRefs
  let (values, status) = getLogic state (map fromIntegral varRefs') toVarValF
   in case values of
        Nothing -> pure . FMIT.statusToCInt $ status
        Just values' -> pokeArray varVals values' >> (pure . FMIT.statusToCInt) status

-- | The function getLogic extracts the relevant values from the user state
...
getLogic state vRefs valConvF =
  let outputSVs = HM.elems . FMIT.fcVars $ state
      values = getVsFromVRefs outputSVs vRefs valConvF
   in case values of
